...
\huge Priority Queues - Heap Analysis \\ % The assignment title
...
\begin{enumerate}
	\item Implement a priority queue naively by using one of our other data structures (e.g., list, tree, or hash table). You may use Java's built in data structures (e.g., HashMap) to make this easier if you'd like.
	\item Run an experiment where you test the performance of your MinHeap to the naive implementation of the priority queue. Make sure you test the methods push(), poll(), and peek(). Which operations are slower? Which are faster? Do the results you see match the theoretical (i.e., big-theta runtimes) for the respective methods? Why or why not?
	\item \textbf{FILES TO DOWNLOAD:} None
	\item \textbf{FILES TO SUBMIT:} HeapAnalysis.pdf
\end{enumerate}
